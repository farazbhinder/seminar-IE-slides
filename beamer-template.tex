...
\begin{block}{Toponyms}
	Words in a document text that correspond to \textbf{location names}.
\end{block}

refer to a \textbf{populated place}, such as: \underline{city}, \underline{state/province}, \underline{country}\\
%\begin{itemize}
%	\item Cities/Towns
...
%\bigskip
%\pause
%e.g.  ``\underline{\textbf{France}} won the world cup in 1998."\\
\bigskip
\pause
e.g. ``\textbf{Faisalabad} is the third-most-populous city in \textbf{Pakistan}, and the second-largest in the eastern province of \textbf{Punjab}."
} % END OF FRAME
...
\bigskip
\pause
step 1: detecting the toponyms = Toponym recognition\\
	``\underline{\textbf{France}} won the world cup in 1998."\\
\bigskip
\pause
step 2: grounding the detected toponyms = Toponym resolution\\
``\underline{France} \textbf{(lat 46, long 2)} won the world cup in 1998."\\
...
\frame{
	...
	\begin{figure}
		...
		\caption{GeoNames gazeteer \textit{Source: \url{http://geonames.org/}}}
	\end{figure}
...
%		\end{figure}
		...
		\begin{figure}
			...
			\caption{from AIDA Web Interface \textit{Source: \url{https://gate.d5.mpi-inf.mpg.de/webaida/}}}
		\end{figure}
	
		\onslide<2>
		\begin{figure}
			\includegraphics[width=\textwidth]{ambi.png} 
			\caption{from Ambiverse Entity Linking Demo \textit{Source: \url{https://www.ambiverse.com/\#demo}}}
		\end{figure}
...
\frame{
	...
	\begin{itemize}
		\item rule based (lookups)
		\item statistical methods (NER, POS)
	...
	\begin{columns}
		...
		\begin{column}{.7\textwidth}
			...
			\begin{figure}
				...
				\caption{Dateline toponym example., \textit{Source: \url{https://www.nytimes.com/2017/02/03/world/europe/kjell-magne-bondevik-norway-dulles.html?_r=0}}}
			\end{figure}
		\end{column}
...
\frame{
	...
	the other \textbf{toponyms} in the \textbf{window around toponym t}
	\end{block}
	...
	\begin{overprint}
		...
		\begin{figure}
			...
			\caption{Adaptive Context Features for Toponym Resolution in Streaming News [\text{1}], \textit{Source: \url{http://www.cs.umd.edu/~hjs/pubs/toponymresolution-sigir2012.pdf}}}
		\end{figure}
...
\frame{
	...
	\begin{itemize}
		\item news article have many proximate toponyms
	\end{itemize}
...
\frame{
	...
	\begin{itemize}
		\item \underline{datasets}: news dataset of ACE, CLUST and LGL
		\item \underline{comparison}: two commercial geotagging frameworks i.e. (Thomson Reuter's OpenCalais and Yahoo!'s Placemaker)
		\item \underline{evaluation measures}: precision, recall, F1 score
	\end{itemize}
...
\frame{
	...
	trend in search engines to \textbf{answer some queries directly}, instead of returning search results (Leidner [\text{2}]) \\
...
\frame{
	...
	\begin{itemize}
		\item \textbf{14.8\%} queries had toponym in them - 2001 Excite search engine log (Sanderson and Kohler [\text{5}])
		\bigskip
		\item \textbf{13\%} geographic queries (related to USA) in 3 months data of AOL search engine in 2006 (Gan et al. [\text{6}])
	\end{itemize}
...
\frame{
	...
	geographically relevant documents might not be returned using keyword based search (Vaid et al. [\text{4}])\\
	...\\
...
\frame{
	...
	\begin{itemize}
		\item[\text{[1]}]  Michael D. Lieberman and Hanan Samet. Adaptive Context Features for Toponym Resolution in Streaming News. In In SIGIR'12: Proceedings of the 35th International ACM SIGIR Conference on Research and Development in Information Retrieval, pages 731$-$740, 2012.
		\item[\text{[2]}] Jochen L Leidner. Toponym Resolution in Text : Annotation, Evaluation and Applications of Spatial Grounding of Place Names. Universal Press, Boca Raton, FL, USA, 2008.
		\item[\text{[3]}] Michael D Lieberman, Hanan Samet, and Jagan Sankaranarayanan. Geotagging with local lexicons to build indexes for textually-specified
...
\frame{
	...
	\begin{itemize}
		\item[\text{[4]}]  Subodh Vaid, Christopher B. Jones, Hideo Joho, and Mark Sanderson. Spatio-Textual Indexing for Geographical Search on the Web. In Proceedings of the 9th International Conference on Advances in Spatial and Temporal Databases, SSTD'05, pages 218$-$235, Berlin, Heidelberg, 2005. Springer-Verlag.
		\item[\text{[5]}] Mark Sanderson and Janet Kohler. Analyzing Geographic Queries. In	SIGIR Workshop on Geographic Information Retrieval, 2004.
		\item[\text{[6]}]  Qingqing Gan, Josh Attenberg, Alexander Markowetz, and Torsten Suel. Analysis of Geographic Queries in a Search Engine Log. In Pro	ceedings of the First International Workshop on Location and the Web, LOCWEB '08, pages 49$-$56, New York, NY, USA, 2008. ACM.
	\end{itemize}
